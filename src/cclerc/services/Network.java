...
public class Network {
    ...
    public static int isReachable(String aInIp, int aInPort, NetworkInterface aInNetworkInterface, int aInTimeout, int aInMaxRetries, boolean aInUseProxy) {
        ...
        while ((lRetries <= aInMaxRetries) && (!lIsReachable)) {
            ...
            boolean lIsProxyReachable = true;
            if (!lProxy.equals(Proxy.NO_PROXY)) {
                try (Socket lSocket = new Socket()) {
                    ...
                    lIsProxyReachable = true;
                } catch (Exception e) {
                    lIsProxyReachable = false;
                    lRetries++;
                    Utilities.sleep(500);
                }
            }
            ...
            if (lIsProxyReachable) {
                try (Socket lSocket = new Socket(lProxy)) {
                    lSocket.bind(new InetSocketAddress(lInetAddress, 0));
